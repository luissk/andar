...
class PresupuestoModel extends Model{
    ...
    public function insertarPresupuesto($nroPre,$idusuario2,$cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$arrDT,$verP,$tcambio,$pentrega,$fpago,$voferta,$lentrega){
        $query = "insert into presupuesto(pre_numero,pre_fechareg,idusuario2,idcliente,pre_porcenprecio,pre_porcsem,pre_periodo,pre_periodonro,pre_piezas,pre_verpiezas,pre_tcambio,pre_pentrega,pre_fpago,pre_voferta,pre_lentrega,pre_status) 
        values(?,now(),?,?,?,?,?,?,?,?,?,?,?,?,?,1)";

        $st = $this->db->query($query, [$nroPre,$idusuario2,$cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$arrDT,$verP,$tcambio,$pentrega,$fpago,$voferta,$lentrega]);
    ...
    public function modificarPresupuesto($cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$arrDT,$idpresu,$verP,$nroPre,$tcambio,$pentrega,$fpago,$voferta,$lentrega){
        $query = "update presupuesto set idcliente=?,pre_porcenprecio=?,pre_porcsem=?,pre_periodo=?,pre_periodonro=?,pre_piezas=?,pre_verpiezas=?,pre_numero=?,pre_tcambio=?,pre_pentrega=?,pre_fpago=?,pre_voferta=?,pre_lentrega=? where idpresupuesto = ? and pre_status=1";

        $st = $this->db->query($query, [$cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$arrDT,$verP,$nroPre,$tcambio,$pentrega,$fpago,$voferta,$lentrega,$idpresu]);
    ...
    public function getPresupuestos($desde = '', $hasta = '', $cri = '', $status = [1,2,3]){//1->activo, 2->con guía, 3->devuelto
        ...
        pre.pre_pentrega,pre.pre_fpago,pre.pre_voferta,pre.pre_lentrega,
    ...
    public function getPresupuesto($idpresu, $status = [1,2,3]){
        $query = "select pre.idpresupuesto,pre.pre_numero,pre.pre_fechareg,pre.pre_periodo,pre.pre_periodonro,pre.pre_status,pre.pre_porcenprecio,pre.pre_porcsem,pre.pre_piezas,pre.pre_verpiezas,pre.pre_tcambio,pre.pre_pentrega,pre.pre_fpago,pre.pre_voferta,pre.pre_lentrega,
