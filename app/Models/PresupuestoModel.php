...
class PresupuestoModel extends Model{
    ...
    public function insertarPresupuesto($nroPre,$idusuario2,$cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$verP,$tcambio,$pentrega,$fpago,$voferta,$lentrega,$preciotrans,$nrodias,$preciomyd,$pre_ruc,$pre_glosa){
        $query = "insert into presupuesto(pre_numero,pre_fechareg,idusuario2,idcliente,pre_porcenprecio,pre_porcsem,pre_periodo,pre_periodonro,pre_verpiezas,pre_tcambio,pre_pentrega,pre_fpago,pre_voferta,pre_lentrega,pre_preciotrans,pre_nrodiasm,pre_preciomyd,pre_ruc,pre_glosa,pre_status) 
        values(?,now(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)";

        $st = $this->db->query($query, [$nroPre,$idusuario2,$cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$verP,$tcambio,$pentrega,$fpago,$voferta,$lentrega,$preciotrans,$nrodias,$preciomyd,$pre_ruc,$pre_glosa]);
    ...
    public function modificarPresupuesto($cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$idpresu,$verP,$nroPre,$tcambio,$pentrega,$fpago,$voferta,$lentrega,$preciotrans,$nrodias,$preciomyd,$pre_ruc,$pre_glosa){
        $query = "update presupuesto set idcliente=?,pre_porcenprecio=?,pre_porcsem=?,pre_periodo=?,pre_periodonro=?,pre_verpiezas=?,pre_numero=?,pre_tcambio=?,pre_pentrega=?,pre_fpago=?,pre_voferta=?,pre_lentrega=?,pre_preciotrans=?,pre_nrodiasm=?,pre_preciomyd=?,pre_ruc=?,pre_glosa=? where idpresupuesto = ? and pre_status=1";

        $st = $this->db->query($query, [$cliente,$porcpre,$porcsem,$periodo,$nroperiodo,$verP,$nroPre,$tcambio,$pentrega,$fpago,$voferta,$lentrega,$preciotrans,$nrodias,$preciomyd,$pre_ruc,$pre_glosa,$idpresu]);
